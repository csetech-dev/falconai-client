...
# ── Sizing profiles ──────────────────────────────────────────────────────────
# One compose file serves hosts of very different sizes. All container limits,
# shm_size values and concurrency knobs are ${VAR:-<test default>} in the compose
# files; the profile file supplies the real numbers for THIS host.
#
# Layering (later --env-file wins):
#   .env.sizing.<profile>  vendor defaults, replaced on every bundle update
#   .env.app/.env.storage  operator secrets + per-knob overrides
#   .env.app.runtime       auto-generated derived values (app stack only)
#
# Note that load_env_file also *exports* the operator env file into this shell,
# which outranks every --env-file layer — so an operator override always wins.
# Optional arg: an env file to read FALCON_SIZING_PROFILE from when it is not
# already exported (for the read-only commands that never source one).
# Sets SIZING_ENV_FILE.
resolve_sizing_file() {
  local peek_file="${1:-}"
  local profile="${FALCON_SIZING_PROFILE:-}"

  if [[ -z "${profile}" && -n "${peek_file}" && -f "${peek_file}" ]]; then
    profile="$(sed -n 's/^[[:space:]]*FALCON_SIZING_PROFILE[[:space:]]*=[[:space:]]*//p' "${peek_file}" | tr -d '"'\''[:space:]' | tail -1)"
  fi
  profile="${profile:-test}"

  case "${profile}" in
    test|prod) ;;
    *) die "Unknown FALCON_SIZING_PROFILE='${profile}' (expected: test | prod).
  Set it in .env.app (application stack) or .env.storage (storage stack)." ;;
  esac

  SIZING_ENV_FILE="${ROOT_DIR}/.env.sizing.${profile}"
  [[ -f "${SIZING_ENV_FILE}" ]] || die "Missing sizing profile: ${SIZING_ENV_FILE}
  This file ships with the client bundle. Re-run the update, or copy it from the
  repo root. Compose cannot start without it."

  # Current Compose strips \r itself, but a leading space in a byte-size value
  # (shm_size) still fails at `config` time, older Compose does not strip, and
  # this file may be read by plain `source` elsewhere. Normalize regardless.
  normalize_env_file "${SIZING_ENV_FILE}"
  log "Sizing profile: ${profile} (${SIZING_ENV_FILE##*/})"
}

# Same, but silent — for read-only commands (ps/logs/down) where the profile only
# needs to be in scope so Compose does not warn about unset variables.
resolve_sizing_file_quiet() {
  resolve_sizing_file "$@" >/dev/null
}

# Assemble the docker compose args for the APPLICATION stack.
# Requires ENV_FILE; honours USE_GHCR. Sets COMPOSE_ARGS.
app_compose_args() {
  resolve_sizing_file
  COMPOSE_ARGS=(
    --env-file "${SIZING_ENV_FILE}"
    --env-file "${ENV_FILE}"
    --env-file "${ROOT_DIR}/.env.app.runtime"
    -f "${APP_COMPOSE}"
  )
...
# Assemble the docker compose args for the STORAGE stack.
...
# Args: service, timeout. Uses the already-assembled COMPOSE_ARGS so the sizing
# profile is in scope — a divergent arg set here would emit interpolation warnings.
wait_for_compose_healthy() {
  ...
  while (( elapsed < timeout )); do
    local cid
    cid="$("${COMPOSE[@]}" "${COMPOSE_ARGS[@]}" ps -q "${service}" 2>/dev/null || true)"
    if [[ -n "${cid}" ]]; then
      local status
...
write_deploy_runtime_env_file() {
  ...
  # NOTE: this file is the LAST --env-file, so anything added below outranks both
  # the sizing profile and the operator's .env.app. Keep it to derived host/URL
  # values only — never add a FALCON_SZ_* or concurrency key here.
  cat > "${target}" <<EOF
